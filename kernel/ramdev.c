...
int ramdisk_write(struct rfs_device *rfs_device, int blkno)
{
  if (blkno < 0 || blkno >= RAMDISK_BLOCK_COUNT)
    kill_current_process("ramdisk_write: write block out of range!\n", -1);
...
{
  if (blkno < 0 || blkno >= RAMDISK_BLOCK_COUNT)
    kill_current_process("ramdisk_read: read block No out of range!\n", -1);
...
struct device *init_rfs_device(const char *dev_name)
{
  ...
  if (!fs_type)
  {
    kill_current_process("No RFS file system found!\n", +1);
  }
  ...
  for (int i = 0; i < RAMDISK_BLOCK_COUNT; ++i)
  {
    ...
    {
      kill_current_process("RAM Disk0: address is discontinuous!\n", -1);
    }
  }
  ...
  if (!rfs_device)
  {
    kill_current_process("RAM Disk0: no free device!\n", -1);
  }
