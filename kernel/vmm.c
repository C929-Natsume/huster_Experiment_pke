...
void *user_va_to_pa(pagetable_t page_dir, void *va)
{
  ...
  pa = (void *)((((*pte >> 10) & 0x3FFFFFFFFFFF) << 12) + ((uint64)va & ((1 << PGSHIFT) - 1)));
  return pa;
}
...
void user_vm_unmap(pagetable_t page_dir, uint64 va, uint64 size, int free)
{
  ...
  if ((!free) && (!size))
  {
    uint64 pa = ((*pte >> 10) & 0x3FFFFFFFFFFF) << 12;
    free_page((void *)pa);
  }
  *pte = 0;
}
...
void user_vm_set_cow(pagetable_t page_dir, uint64 va)
{
  pte_t *pte = page_walk(page_dir, va, 0);
...
pte_t *user_vm_get_pte(pagetable_t pagetable, uint64 va)
{
  return page_walk(pagetable, va, 0);
}

void user_vm_map_cow(pagetable_t page_dir, uint64 va, uint64 size, uint64 pa)
{
  uint64 first, last;
  pte_t *pte;
  uint64 perm = PTE_R | PTE_A | PTE_U | PTE_COW | PTE_V; // read-only, COW
  for (first = ROUNDDOWN(va, PGSIZE), last = ROUNDDOWN(va + size - 1, PGSIZE);
       first <= last; first += PGSIZE, pa += PGSIZE)
  {
    if ((pte = page_walk(page_dir, first, 1)) == 0)
