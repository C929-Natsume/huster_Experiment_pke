...
ssize_t do_exec(char *pathpa)
{
  int fd;
  elf_header ehdr;
  elf_prog_header phdr;
  process *proc = current;
  pagetable_t pagetable = proc->pagetable;
  struct file *f;

  fd = do_open(pathpa, O_RDONLY);
  if (fd < 0)
    return -1;

  f = &(current->pfiles->opened_files[fd]);
  if (f->status != FD_OPENED || !f->readable)
  {
    do_close(fd);
    return -1;
  }
  ...
  {
    int st = proc->mapped_info[i].seg_type;
    if (st == CODE_SEGMENT || st == DATA_SEGMENT ||
        st == STACK_SEGMENT || st == HEAP_SEGMENT)
    {
      uint64 va = proc->mapped_info[i].va;
      uint32 npages = proc->mapped_info[i].npages;
      for (uint32 k = 0; k < npages; k++)
        user_vm_unmap(pagetable, va + k * PGSIZE, PGSIZE, 1);
    }
  }
  ...
  {
    int i;
    for (i = 0; i < proc->total_mapped_region; i++)
    {
      if (proc->mapped_info[i].seg_type == HEAP_SEGMENT)
      {
        proc->mapped_info[i].va = 0;
        proc->mapped_info[i].npages = 0;
        proc->total_mapped_region = i;
        break;
      }
    }
  }
  ...
  memset((void *)user_stack_pa, 0, PGSIZE);
  user_vm_map(pagetable, USER_STACK_TOP - PGSIZE, PGSIZE, user_stack_pa,
              prot_to_type(PROT_READ | PROT_WRITE, 1));
  for (int i = 0; i < proc->total_mapped_region; i++)
  {
    if (proc->mapped_info[i].seg_type == STACK_SEGMENT)
    {
      proc->mapped_info[i].va = USER_STACK_TOP - PGSIZE;
      proc->mapped_info[i].npages = 1;
      break;
    }
  }
  ...
  // Load ELF segments
  for (int i = 0, off = ehdr.phoff; i < ehdr.phnum; i++, off += sizeof(phdr))
  {
    if (vfs_lseek(f, off, LSEEK_SET) != (ssize_t)off)
      continue;
    if (vfs_read(f, (char *)&phdr, sizeof(phdr)) != sizeof(phdr))
      continue;
    if (phdr.type != ELF_PROG_LOAD)
      continue;
    if (phdr.memsz < phdr.filesz || phdr.vaddr + phdr.memsz < phdr.vaddr)
    {
      do_close(fd);
      return -1;
    }
    if (phdr.memsz > PGSIZE)
    {
      do_close(fd);
      return -1;
    }
    ...
    memset(pa, 0, PGSIZE);
    uint64 perm = prot_to_type(PROT_READ | PROT_WRITE | PROT_EXEC, 1);
    if (phdr.flags == (SEGMENT_READABLE | SEGMENT_EXECUTABLE))
      perm = prot_to_type(PROT_READ | PROT_EXEC, 1);
    else if (phdr.flags == (SEGMENT_READABLE | SEGMENT_WRITABLE))
      perm = prot_to_type(PROT_READ | PROT_WRITE, 1);

    user_vm_map(pagetable, phdr.vaddr, PGSIZE, (uint64)pa, perm);
    if (vfs_lseek(f, phdr.off, LSEEK_SET) != (ssize_t)phdr.off)
    {
      do_close(fd);
      return -1;
    }
    if (vfs_read(f, (char *)pa, phdr.filesz) != (ssize_t)phdr.filesz)
    {
      do_close(fd);
      return -1;
    }
    ...
    for (j = 0; j < PGSIZE / sizeof(mapped_region); j++)
      if (proc->mapped_info[j].va == 0)
        break;
    proc->mapped_info[j].va = phdr.vaddr;
    ...
    {
      proc->mapped_info[j].seg_type = DATA_SEGMENT;
...

  {
    ...
    for (j = 0; j < PGSIZE / sizeof(mapped_region); j++)
      if (proc->mapped_info[j].va == 0)
        break;
    proc->mapped_info[j].va = USER_FREE_ADDRESS_START;
    proc->mapped_info[j].npages = 0;
    proc->mapped_info[j].seg_type = HEAP_SEGMENT;
    proc->total_mapped_region++;
  }

  proc->trapframe->epc = ehdr.entry;
