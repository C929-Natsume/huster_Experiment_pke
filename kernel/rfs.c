...
int rfs_format_dev(struct device *dev)
{
  ...
  if (rfs_w1block(rdev, RFS_BLK_OFFSET_SUPER) != 0) // write to device
    kill_current_process("RFS: failed to write superblock!\n", -1);
  ...
  for (int inode_block = 0; inode_block < RFS_MAX_INODE_BLKNUM; ++inode_block)
  {
    if (rfs_w1block(rdev, RFS_BLK_OFFSET_INODE + inode_block) != 0)
      kill_current_process("RFS: failed to initialize empty inodes!\n", -1);
  }
...
int rfs_alloc_block(struct super_block *sb)
{
  ...
  if (free_block == -1)
    kill_current_process("rfs_alloc_block: no more free block!\n", -1);
  return free_block;
}
                 ...
                 int *offset)
{
  ...
  if (f_inode->size < *offset)
    kill_current_process("rfs_read:offset should less than file size!", -1);
                  ...
                  int *offset)
{
  if (f_inode->size < *offset)
  {
    kill_current_process("rfs_write:offset should less than file size!", -1);
  }
  ...
  for (int i = 0; i < total_direntrys; ++i)
  {
    ...
    { // found
      ...
      if (rfs_update_vinode(child_vinode) != 0)
        kill_current_process("rfs_lookup: read inode failed!", -1);
      break;
    }
    ++p_direntry;
  }
...
struct vinode *rfs_create(struct vinode *parent, struct dentry *sub_dentry)
{
  ...
  if (free_dinode == NULL)
    kill_current_process("rfs_create: no more free disk inode, we cannot create file.\n", -1);
  ...
  // Note: DO NOT DELETE CODE BELOW kill_current_process.
  free_dinode->size = 0;
...
int rfs_hook_opendir(struct vinode *dir_vinode, struct dentry *dentry)
{
  ...
  for (int i = dir_vinode->blocks - 1; i >= 0; i--)
  {
    ...
    if (previous != NULL && previous - pdire != RFS_BLKSIZE)
      kill_current_process("rfs_hook_opendir: memory discontinuity", -1);
...
int rfs_readdir(struct vinode *dir_vinode, struct dir *dir, int *offset)
{
  ...
  // note: DO NOT DELETE CODE BELOW kill_current_process.
  memcpy(dir->name, p_direntry->name, RFS_MAX_FILE_NAME_LEN + 1);
...
struct vinode *rfs_mkdir(struct vinode *parent, struct dentry *sub_dentry)
{
  ...
  if (free_dinode == NULL)
    kill_current_process("rfs_mkdir: no more free disk inode, we cannot create directory.\n", -1);
...
struct super_block *rfs_get_superblock(struct device *dev)
{
  ...
  if (rfs_r1block(rdev, RFS_BLK_OFFSET_SUPER) != 0)
    kill_current_process("RFS: failed to read superblock!\n", -1);
  ...
  if (sb->magic != RFS_MAGIC)
    kill_current_process("rfs_get_superblock: wrong ramdisk device!\n", -1);
  ...
  if (rfs_r1block(rdev, RFS_BLK_OFFSET_BITMAP) != 0)
    kill_current_process("RFS: failed to read bitmap!\n", -1);
